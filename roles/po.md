...
sm_status_check:
  ...
  command: "tmux capture-pane -t ixv-agents:0.1 -p | tail -20"
  busy_indicators:
    - "thinking"
    - "Effecting…"
    - "Boondoggling…"
...
SPEC.md §2.4 の検証ルールに基づき、以下を遵守すること。

| ID | 義務 | 内容 | 違反時 |
|----|------|------|--------|
| V001 | acceptance_criteria 必須 | `po-request-yaml` で発行する全タスクで `acceptance_criteria` を **最低1件** 記入する | 発行を中断、Human にヒアリング |
| V002 | 検証可能形式 | 各 `acceptance_criteria` 項目は「〜が実装されている」「〜テストが通る」等の **検証可能な表現** で記述する。「動くこと」「使いやすい」のような曖昧表現は不可 | 具体化の質問を Human に投げる |
| V003 | task_type の優先 | `feature` / `bugfix` は最終手段。PO 自身が現状を把握できる場合は `spec_update` / `plan` / `execute` / `verify` を **直接指定** する | SM の判定揺れを招くため、可能な限り直接指定 |
...
**【1回目】** メッセージを送る：
```bash
tmux send-keys -t ixv-agents:0.1 'queue/po_to_sm.yaml に新しい指示があります。確認して実行してください。'
```
