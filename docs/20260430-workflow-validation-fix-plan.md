...
各修正の効果は、`workspace/queue/` 配下に **検証用の異常系 YAML** を投入し、各 SKILL が期待通りに拒否/差し戻しを行うかで確認する。

| 検証ケース | 投入ファイル | 期待結果 |
|---|---|---|
| 空 acceptance_criteria | po_to_sm.yaml に `acceptance_criteria: []` | po-request-yaml が発行を拒否 |
| 空 definition_of_done | tasks/dev1.yaml に `definition_of_done: []` | sm-write-task-yaml が発行を拒否 |
| 未完了依存 | tasks/dev1.yaml に未完了 task_id を `dependencies` に記載 | sm-write-task-yaml が発行を拒否、または Dev が blocked 報告 |
| 循環依存 | A→B, B→A の2タスクを発行 | sm-write-task-yaml が循環を検出して拒否 |
| DoD 未カバー done | reports に DoD 3項目のうち2項目しか changes/artifacts に無い | dev-write-report が issues に記載、sm-scan-reports が needs_review に格下げ |
| feature 受領（未仕様） | po_to_sm.yaml に `task_type: feature` で README に未記載の機能 | sm-receive-request が Phase 2 と判定 |
