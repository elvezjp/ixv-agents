...
本マトリクスは契約上の権限であり、ファイルシステム強制ではない。ユーザの手動編集や外部書き換えは可能で、SM の内部状態と乖離して**静かに上書き**されるおそれがある。

- リカバリは **Git を第一義** とする（`git diff` / `git revert` / `git restore`）。未コミットの手動編集は失われ得る。
- 手動編集時は SM を停止してから編集し、完了後に再起動して再同期する。
- 不整合検知時、SM は処理を中断し PO 経由でユーザに報告する。自動修復はしない。

技術的検証メカニズム（`revision` / `updated_by` 等）は現バージョンでは導入せず、SM 多重化や HITL 高頻度運用時に再検討する。
