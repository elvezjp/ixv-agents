...
- **命名規則**: セッション名は、AIエージェントが宛先を正しく判断できるようにするため、役割がわかる名前にする。
...
- tmuxセッション（`ixv-manage`, `ixv-dev`）を作成する
- 各エージェント（PO, SM, Dev1-3）のペインを起動する
- AI CLI（OpenCode / Claude Code）を初期化する
