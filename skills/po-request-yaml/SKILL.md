...
### Step 4.5: acceptance_criteria 必須項目検証

**SPEC.md §2.4.2 に基づく必須検証**。書き込み前に以下を **Step として実行** すること。

#### 検証1: 空配列禁止

`acceptance_criteria` が **空配列** であってはならない。

```
if len(acceptance_criteria) == 0:
    YAML 生成を中断
    Human に「完了条件は最低1件必要です。どうなったらOKですか？」とヒアリング
    ヒアリング結果を反映してから Step 4 をやり直す
```

#### 検証2: 検証可能形式

各 `acceptance_criteria` 項目は **検証可能な形式** で記述すること。

| 形式 | 例 |
|------|---|
| Good（検証可能） | 「〜が実装されている」「〜テストが通る」「〜が表示される」「〜エンドポイントが動作する」 |
| Bad（曖昧） | 「動くこと」「使いやすい」「いい感じになる」「対応する」 |

曖昧な項目を発見した場合：

```
Human に「『{曖昧な項目}』はどうなったら満たされたとみなせますか？」と質問
具体化された表現に書き換えてから Step 4 をやり直す
```

#### 違反時の挙動（SPEC.md §2.4.2 と同期）

| 違反内容 | アクション |
|---------|----------|
| 空配列 | 発行を中断し、Human にヒアリング |
| 曖昧な表現 | Human に具体化を確認 |

検証をすべてパスした場合のみ、Step 5 に進む。
...
- **feature / bugfix の発行は最終手段**: SPEC.md §2.6.2 の決定木で SM が自動判定するが、
  PO 自身が現状を把握できる場合は、より具体的な task_type（`spec_update` / `plan` / `execute` / `verify`）を直接指定することが望ましい。判定の手間と曖昧さを減らすことができる。
